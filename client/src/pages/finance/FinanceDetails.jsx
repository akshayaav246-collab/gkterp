...
const FinanceDetails = () => {
    ...
    const defaultDetailed = {
        ...
        invoiceNumber: '',
        invoiceDate: '',
        invoiceValue: 0, // Excl Tax
        invoiceValueWithTax: 0, // Incl GST
        invoiceDocument: '',
        gstType: '',
        gstAmount: 0,
        tdsPercent: 0,
    ...
    const [vendorData, setVendorData] = useState({
        detailed: {
            trainer: { ...defaultDetailed },
            travel: { ...defaultDetailed },
            accommodation: { ...defaultDetailed },
            venue: { ...defaultDetailed },
            courseMaterials: { ...defaultDetailed },
            lab: { ...defaultDetailed },
            royalty: { ...defaultDetailed },
            marketing: { ...defaultDetailed }
        },
    ...
    const fetchOpportunity = async () => {
        try {
            ...
            Object.keys(defaultDetailed).forEach(key => {
                // Ensure default structure
            });

            Object.keys(categoryLabels).forEach(key => {
                newDetailed[key] = { ...defaultDetailed, ...(detailed[key] || {}) };
                // Format dates
    ...
    useEffect(() => {
        ...
        if (
            gstVal !== clientData.gstAmount ||
            tdsVal !== clientData.tds ||
            totalInv !== clientData.totalInvoiceAmount ||
            receivable !== clientData.amountReceivable
        ) {
            setClientData(prev => ({
                ...prev,
                gstAmount: gstVal,
    ...
    const handleDetailedChange = (category, field, value) => {
        setVendorData(prev => {
            const catData = { ...prev.detailed[category], [field]: value };
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                                    <div>
                                        ...
                                        <input
                                            type="number"
                                            name="invoiceAmount"
                                            value={clientData.invoiceAmount}
                                    ...
                                    <div>
                                        ...
                                        <input
                                            type="text"
                                            value={clientData.gstAmount.toLocaleString()}
                                            disabled
                                    ...
                                    <div>
                                        ...
                                        <input
                                            type="text"
                                            value={clientData.tds.toLocaleString()}
                                            disabled
                                            className={readOnlyClass}
                                        />
                                    </div>
...
const ExpenseRow = ({
    ...
    return (
        <div className="border border-gray-200 rounded-lg overflow-hidden mb-4">
            ...
            {isExpanded && (
                <div className="p-6 bg-white animate-fadeIn border-t border-gray-200">
                    {/* Vendor & PO */}
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
                        ...
                        <div>
                            ...
                            <select value={data.tdsPercent} onChange={(e) => handleDetailedChange(category, 'tdsPercent', e.target.value)} className={inputClass}>
                                ...
                                {Array.from({ length: 15 }, (_, i) => i + 1).map(val => <option key={val} value={val}>{val}%</option>)}
                            </select>
                        </div>
                        <div>
